...
(deftest internal-trust-absence-is-reported-not-silent
  ...
  ;; the status rides along in the value push ALREADY returns, so an operator can
  ;; sweep the fleet for unconfigured callers without reading anyone's stderr
  (seed-log!)
  (with-redefs [br/internal-trust (constantly nil)]
    (is (= :unconfigured (:internal-trust (br/push tmp)))))
  (with-redefs [br/internal-trust (constantly synthetic-trust)]
    (is (= :configured (:internal-trust (br/push tmp)))))
  (clean!))
